...
{
	public class DaemonCopyDetailView : BaseView<DaemonCopyDetailView>
	{
		...
		public override void RegisterUpdateHandler()
		{
			...
			Singleton<DaemonIslandMode>.Instance.dataUpdated += UpdateDaemonCopyInfo;
		...
		protected override void HandleAfterOpenView ()
		{
			...
			copyPointId = Singleton<DaemonIslandMode>.Instance.selectedCopyPoint;
		...
		//恶魔岛副本信息更新回调
		private void UpdateDaemonCopyInfo(object sender,int code)
		{
		...
		private void UpdateMaxFightTimes()
		{
			...
			maxFightNum = (myVigour / copyVigour) > remainCopyTimes? remainCopyTimes: (myVigour / copyVigour);
			maxFightNum = maxFightNum > 10 ? 10 : maxFightNum;
		...
		{
			//扫荡功能状态
			if (copyPointId == Singleton<DaemonIslandMode>.Instance.NewOpenMapId)
			{
		...
		private void ShowMonsters(List<Transform> monList, string[] sprNameStr, UIGrid grid)
		{
			for (int i = 0; i < monList.Count; ++i)
			{
				...
				{
//					monList[i].FindChild("touxiang").GetComponent<UISprite>().spriteName = sprNameStr[i];
...
//				Singleton<DaemonIslandView>.Instance.CloseView();
		...
		private void FastFight1OnClick(GameObject go)
		{
			...
			{
				if (remainCopyTimes > 0)
				{
					if (MeVo.instance.vigour >= BaseDataMgr.instance.GetMapVo(copyPointId).vigour)
					{
						Singleton<DaemonIslandMode>.Instance.fightNum = 1;
						Singleton<DaemonCopyFastFightView>.Instance.OpenView();
					}
					else
					{
						OpenBuyVigourTips();
				...
				{
					MessageManager.Show(LanguageManager.GetWord("CopyDetailView.noCopyTimes"));
				}
			}
			else
			{
		...
		private void FastFight10OnClick(GameObject go)
		{
			if (MeVo.instance.vip >= BaseDataMgr.instance.GetVIPLowestGrade(17))
			{
				if (maxFightNum > 0)
				{
					Singleton<DaemonIslandMode>.Instance.fightNum = maxFightNum;
					Singleton<DaemonCopyFastFightView>.Instance.OpenView();
				}
				else
				{
