...
{
    static public UICheckboxX current;
    ...
    void Awake()
    {
        ...
        if (radioButtonRoot == null)    ////直接查找，省去拖拽的操作
            radioButtonRoot = transform.parent;
        if (label == null && transform.FindChild("label"))    ////直接查找，省去拖拽的操作
            label = transform.FindChild("label").GetComponent<UILabel>();
        
        if (checkSprite==null&&transform.FindChild("light"))   //直接查找，省去拖拽的操作
            checkSprite = transform.FindChild("light").GetComponent<UISprite>();
    ...
    void Set(bool state)
    {
        ...
        else if (mChecked != state)
        {
            ...
            {
                UICheckboxX[] cbs = radioButtonRoot.GetComponentsInChildren<UICheckboxX>(true);

                for (int i = 0, imax = cbs.Length; i < imax; ++i)
                {
                    UICheckboxX cb = cbs[i];
            ...
            if (label != null)
            {
                label.color = state ? Color.white : NGUITools.ParseColor("EAAE8F", 0);
            }
            current = this;
