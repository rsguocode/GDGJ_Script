...
{
    public class MonsterAiController : AiControllerBase
    {
        ...
        private void Update()
        {
            ...
            if (MeController.StatuController.CurrentStatu == Status.IDLE)
            {
                ...
                {
                    AiSearchBehaviour(); // 优先级比 AiCruiseBehaviour 高
                    _stopTime = 0;
                    return;
                }
        ...
        {
            if (_meEnemyVo == null || _meEnemyVo.MonsterVO.searchLength == 0) return;
            Vector3 point = AppMap.Instance.me.GoBase.transform.position; // 主角位置
            float attackRange1 = _meEnemyVo.MonsterVO.sight_range * 0.001f - 0.1f; // sight_range; 攻击范围
            float attackRange2 = 2 * _meEnemyVo.MonsterVO.sight_range * 0.001f - 0.1f;
            bool needFellow = false;
            //if (_selfTransform.position.x < point.x - attackRange1) // 主角在 右边
            if (_selfTransform.position.x < point.x) // 主角在 右边, 应该加个 ‘可视范围’
            {
                if (_selfTransform.position.x + _meEnemyVo.MonsterVO.searchLength < point.x - attackRange2)
                {// 主角在attack2攻击范围‘外’,距离远一点
                    point.x = MeController.transform.position.x + _meEnemyVo.MonsterVO.searchLength; // 往右走, value:3
                }
                else if (_selfTransform.position.x + _meEnemyVo.MonsterVO.searchLength < point.x - attackRange1)
                {// 主角在attack1攻击范围‘外’,attack2‘内’，距离近一点
                    int random = Random.Range(0, 1000);
                    if (random > _meEnemyVo.MonsterVO.attackAccuracy) // attackAccuracy; 攻击精度,如 800
                    {
                        point.x = _selfTransform.position.x + _meEnemyVo.MonsterVO.searchLength; // 往右走
                    }
                    else // 这个范围内，怪可以攻击了
                    {
                        point.x -= attackRange1;
                    }

                    point.y = point.y + Random.Range(-50, 50) * 0.01f;
                }
                else // 主角在attack1攻击范围‘内’
                {
                    int random = Random.Range(0, 1000);
                    if (random > _meEnemyVo.MonsterVO.attackAccuracy)//攻击精度
                    {
                        //float x1 = point.x - attackRange1;
                        //float x2 = Mathf.Max(_selfTransform.position.x, point.x - attackRange2);
                        //point.x = Random.Range(x1, x2);
            ...
            //else if (_selfTransform.position.x > point.x + attackRange1) // 主角在 左边
            else if (_selfTransform.position.x > point.x) // 主角在 左边
            {
                if (_selfTransform.position.x - _meEnemyVo.MonsterVO.searchLength > point.x + attackRange2)
                {
                    point.x = _selfTransform.position.x - _meEnemyVo.MonsterVO.searchLength;
                }
                else if (_selfTransform.position.x - _meEnemyVo.MonsterVO.searchLength > point.x + attackRange1)
                {
                    int random = Random.Range(0, 1000);
                    if (random > _meEnemyVo.MonsterVO.attackAccuracy)
                    {
                        point.x = _selfTransform.position.x - _meEnemyVo.MonsterVO.searchLength;
                    }
                    else
                    ...
                    point.y = point.y + Random.Range(-120, 120) * 0.01f;
                }
                else
                {
                    int random = Random.Range(0, 1000);
                    ...
                    {
                        //float x1 = point.x + attackRange1;
                        //float x2 = Mathf.Min(_selfTransform.position.x, point.x + attackRange2);
                        //point.x = Random.Range(x1, x2);
