...
namespace PCustomDataType
{
    ...
    public void read(MemoryStream msdata)
    {
        ...
        sprite = proto_util.readUByte(msdata);
    ...
    public void write(MemoryStream msdata)
    {
        ...
        proto_util.writeUByte(msdata, sprite);
        proto_util.writeUInt(msdata, prize);
        proto_util.writeUInt(msdata, num);
    }
    
    public static void readLoop(MemoryStream msdata, List<PWandSuperInfo> p)
        {
            int Len = proto_util.readShort(msdata);

            for (int i = 0; i < Len; i++)
            {
                PWandSuperInfo _pm = new PWandSuperInfo();
                _pm.read(msdata);
                p.Add(_pm);
            }
        }

        public static void writeLoop(MemoryStream msdata, List<PWandSuperInfo> p)
        {
            proto_util.writeShort(msdata, (short)p.Count);

            foreach (PWandSuperInfo ps in p) ps.write(msdata);
        }
